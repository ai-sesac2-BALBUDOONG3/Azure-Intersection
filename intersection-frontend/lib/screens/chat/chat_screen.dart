...
class _ChatScreenState extends State<ChatScreen> {
  ...
  Future<void> _sendMessage() async {
    ...
    try {
      final newMessage =
          await ApiService.sendChatMessage(widget.roomId, content);
  ...
  /// "보낼 수 없음" 안내 다이얼로그 (차단/신고 상태에서 전송 시)
  void _showSendBlockedDialog() {
    String message;
    if (_iReportedThem) {
      ...
      appBar: AppBar(
        ...
        actions: [
          PopupMenuButton<String>(
            ...
            itemBuilder: (context) => [
              // 차단하지 않았고 신고 안했고, 상대가 나를 차단하지 않았을 때만 "차단하기"
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                const SizedBox(width: 12),
                GestureDetector(
                  onTap: _isSending || _isBlocked || _iReportedThem
                      ? null
                      : _sendMessage,
                  child: Container(
                    width: 48,
                    height: 48,
                    decoration: BoxDecoration(
                      color: _isSending || _isBlocked || _iReportedThem
                          ? Colors.grey
                          : Colors.blue,
          ...
          actions: [
            ...
            TextButton(
              onPressed: () async {
                ...
                final success =
                    await ApiService.blockUser(widget.friendId);

                if (success && mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content:
                          Text('${widget.friendName}님을 차단했습니다'),
                    ),
                  );
          ...
          actions: [
            ...
            TextButton(
              onPressed: () async {
                ...
                final success =
                    await ApiService.deleteChatRoom(widget.roomId);
          ...
          actions: [
            ...
            TextButton(
              onPressed: () async {
                ...
                final success =
                    await ApiService.unblockUser(widget.friendId);

                if (success && mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text(
                          '${widget.friendName}님의 차단을 해제했습니다'),
                    ),
                  );
                  await _checkBlockStatus();
                }
          ...
          actions: [
            ...
            TextButton(
              onPressed: () async {
                ...
                if (_reportId != null) {
                  final success =
                      await ApiService.cancelReport(_reportId!);
